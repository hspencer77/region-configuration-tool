...
def get_options():
    ...
    def format_error(err_string):
        ...
        """
        msg = ("\nIncorrect Region \'key=value\' format - " +
               err_string + "\nPlease use "
    ...
    def region_check(region):
        ...
        for param in [region_name, cloud_cert, domain_name]:
            ...
            if not any(key_param in key for key in
                       ["region_name", "cloud_cert", "domain_name"]):
                msg = format_error(region)
                raise argparse.ArgumentTypeError(msg)

        # Check for proper length and valid region name
        region_key, region_value = region_name.split('=')
        if not re.match("^(?![0-9]+$)(?!-)[a-zA-Z0-9-]{,63}(?<!-)$",
                        region_value):
            msg = format_error(region)
            region_msg = ("\n\n" + region_key +
                          " has a value which is not a valid DNS label." +
        ...
        if not os.path.isfile(cloud_value):
            msg = format_error(region)
            cloud_msg = ("\n\n" + cloud_key +
                         " has a value which is not a file.")
        ...
        if not re.findall(
            ...
            msg = format_error(region)
            domain_msg = ("\n\n" + domain_key +
                          " has a value which is not a value DNS domain. " +
                          "Please refer to the " +
                          "<domain> definition in RFC 1035 " +
                          "https://www.ietf.org/rfc/rfc1035.txt.")
    ...
    def digest_check(digest):
        ...
        Formats supported by Eucalyptus Federation:
            - sha1
            - sha224
            - sha256
            - sha384
    ...
    parser = argparse.ArgumentParser(prog="region-config-tool.py",
                                     ...
                                     description='Script that creates region \
                                     configuration file from \
                                     multiple region (i.e. cloud) parameters \
                                     that are intended to be \
                                     part of a federated environment.')
    region_group = parser.add_argument_group("Region Arguments",
                                             "Arguments to " +
    ...
    region_group.add_argument('region', nargs='+', type=region_check,
                              ...
                              region_name=ParameterValue,cloud_cert=ParameterValue,\
                              domain_name=ParameterValue.\
                              For each cloud (i.e. region) - \
                              \"region_name\" should match \
                              region.region_name cloud property; \
                              \"cloud_cert\" should be \
                              the cloud certificate \
                              (/var/lib/eucalyptus/keys/cloud-cert.pem) \
                              of the Cloud Controller \
                              for that cloud (i.e. region);\
                              \"domain_name\" should match the \
                              system.dns.dnsdomain cloud property')
    file_group = parser.add_argument_group("Region Configuration " +
                                           "File Argument",
                                           'Argument to define name of \
                                           of generated region \
                                           configuration file.')
    file_group.add_argument('-f', '--filename', dest='file_name',
                            type=argparse.FileType('w'),
                            help='Generated region \
                            configuration file.')
    http_option = parser.add_argument_group("HTTP/HTTPS Protocol " +
                                            "Flag Argument",
                                            'Argument to set HTTP or HTTPS \
                                            for Service Endpoints')
    http_option.add_argument('-p', '--protocol', dest='protocol',
                             type=http_check, default='http',
                             help='Flag to use HTTP/HTTPS \
                             for Service Endpoints. Default: http')
    cert_fingerprint = parser.add_argument_group("OpenSSL Signature " +
                                                 "Algorithm Argument",
                                                 'Argument that defines what \
                                                 OpenSSL signing alorithm \
                                                 was used for the \
                                                 cloud certificate')
    cert_fingerprint.add_argument('-d', '--digest', dest='digest',
                                  type=digest_check, default='sha256',
                                  help="Digest algorithm used to sign cloud " +
                                  "certificate. Default: sha256")
